...
const SERVICES = [
    ...
    sections: [
      {
        ...
        badge: "إلزامية",
        summary: "هذه المرحلة هي حجر الأساس في المخيم، وتهل إلى أي تخصص تقني.",
      },
      {
        title: "المرحلة الثانية: التمكين التقني والمسارات التخصصية",
        badge: "إلزامية ثم اختيارية",
        summary: "تنطلق هذه المرحلة بتدريب مكثف للتطبيق العملي.",
      },
      {
        title: "المرحلة الثالثة: صناعة الأثر (TOT)",
        badge: "تأثير",
        summary: "تستهدف هذه المرحلة تحويل المتدربين أمام الجمهور.",
      },
    ],
...

  {
    ...
    desc: "أتقن أدوات العصر وهندسة الأوامر ثم اختر مسارك المفضل واكتسب المهارات العملية اللازمة لتحقيق أحلامك.",
    sections: [
      {
        title: "ريادة الأعمال",
        badge: "🚀",
  ...
  useGSAP(
    () => {
      ...
      ScrollTrigger.getAll().forEach((st) => {
        if (st.trigger && containerRef.current?.contains(st.trigger as Node)) {
          st.kill();
        }
      });
      ...
      >
        {SERVICES.map((svc, i) => (
          ...
          >
            ...
            <span className="pointer-events-none absolute bottom-0 right-4 font-mono text-[22vw] leading-none text-black/5">
              {svc.id}
            </span>
            ...
            <div className="relative z-10 flex w-full max-w-7xl flex-col items-start justify-center gap-8 py-8 md:max-w-full md:flex-row md:items-start md:justify-between md:gap-10 md:py-20">
              ...
              <div className="flex flex-col gap-6 md:w-1/2">
                <span className="heading w-fit -rotate-3 border-2 border-black bg-secondary px-4 py-1 text-center text-xl font-bold text-black shadow-[3px_3px_0_0_#111111]">
                  {svc.category}
                </span>

                <h2 className="heading whitespace-pre-line break-words text-5xl font-black leading-[1.1] tracking-tight text-black sm:text-6xl md:text-7xl lg:text-[4.5rem]">
                  {svc.title}
                  <span className={svc.accent}>.</span>
                </h2>

                <p className="max-w-md text-base text-black/70 md:text-lg">
                  {svc.desc}
                </p>
              </div>
              ...
              <div className="grid w-full grid-cols-1 gap-6 border-t border-black/10 pt-6 lg:grid-cols-2 md:border-l md:border-t-0 md:pl-12 md:pt-0">
                {svc.sections.map((section) => (
                  <div
                    key={section.title}
                    className="border-4 border-black bg-white/60 p-5 shadow-[3px_3px_0_0_#111111]"
                  >
